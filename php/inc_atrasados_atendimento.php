...
if(!empty($cidade)){
	...
	if(strlen($filtros)>2){
		...
		$cidadeCorreto .= ' - '.strtoupper(strtr(strtoupper(utf8_encode(strtoupper($y['nome']))) ,"áéíóúâêôãõàèìòùç","ÁÉÍÓÚÂÊÔÃÕÀÈÌÒÙÇ"));
	}
	else{
...
if(!empty($_POST['data_final'])){
	$data_fim_old = $_POST['data_final'];
	$data_fim = explode("/",$_POST['data_final']);
	$data_fim = $data_fim[2]."-".$data_fim[1]."-".$data_fim[0];
}
elseif(!empty($_GET['data_final'])){
	$data_fim_old = $_GET['data_final'];
	$data_fim = explode("/",$_GET['data_final']);
	$data_fim = $data_fim[2]."-".$data_fim[1]."-".$data_fim[0];
}
else{
	$data_fim = "";	
	...
	$select_query = "
		...
		FROM 
			...
			ordem_montagem o 
		WHERE 
			d.n_montagens = c.n_montagem AND 
			d.n_montagens = o.n_montagem AND 
			o.id_montador <> '0' AND 
			(
				d.data_limite >= '$data_inicio' AND 
				d.data_limite <= '$data_fim'
			) AND 
			d.data_final = '0000-00-00'
			$filtros
	";
	...
	$sql = mysql_query($select_paginacao); // Executa a query no MySQL com o limite de linhas.
		...
		echo "<thead>";
		echo "<tr>";
		echo "<td class=titulo colspan=6>:: Clientes Atrasados em Atedimento ::</td>";
		echo "</tr>";
		echo "<tr>";
		...
		while ($linha = mysql_fetch_array($sql))
		{
			...
			$bairro_old			= $linha["bairro_cliente"];
			...
			echo "<td class='texto'><a href='vis_clientes.php?id_clientes=$id_clientes'>".utf8_decode($bairro_old)."</a></td>";
			if($status == 2){
			####################################### PRE BAIXAS ##################################################
			...
			echo "</tbody>";
			echo "</table>";
		echo "<div align='center'><a href='admim_atrasados_atendimento.php'>Inicio</a></div>";
		echo "<br />";
		echo "<span class='texto'>Mais registros</span>";
		echo "<br />";
			$menos = 0;
			$url = "$PHP_SELF?pagina=$menos&data_inicio=$data_inicio_old&data_final=$data_fim_old&estado=$estado&cidade=$cidade&bairro=$bairro";
		...
		if($pagina > 0) {
			...
			if(!isset($pagina)){
				echo " <a class=\"destaque\" href='$PHP_SELF?pagina=0&data_inicio=$data_inicio_old&data_final=$data_fim_old&estado=$estado&cidade=$cidade&bairro=$bairro'>0</a> ";
			}
			else{
				echo " <a class=\"destaque\" href='$PHP_SELF?pagina=$pagina&data_inicio=$data_inicio_old&data_final=$data_fim_old&estado=$estado&cidade=$cidade&bairro=$bairro'><strong style='font-size:16px;'><u>".$pagina."</u></strong></a> ";
			}
			$mais = $pagina;
		if($pagina < ($paginas - 1)) {
			for($x=0;$x<(($paginas/4)-1);$x++){
				$mais = $mais + 1;
				$url = "$PHP_SELF?pagina=$mais&data_inicio=$data_inicio_old&data_final=$data_fim_old&estado=$estado&cidade=$cidade&bairro=$bairro";
				if($mais<=($paginas-1)){
