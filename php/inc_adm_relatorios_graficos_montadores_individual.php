...
if ((strlen($mont)>0)&&(!empty($data_inicio))&&(!empty($data_fim))){
		$select = "SELECT m.id_montadores, m.nome, o.id_montador, o.n_montagem, o.status, d.n_montagens, d.data_saida_montador FROM clientes c, montadores m, datas d, ordem_montagem o WHERE  c.n_montagem = o.n_montagem AND d.n_montagens = o.n_montagem AND (d.data_saida_montador >= '$data_inicio' AND d.data_saida_montador <= '$data_fim') AND o.id_montador = '$mont' AND m.id_montadores = '$mont' AND c.ativo='$ativo' ORDER BY o.n_montagem ASC";
...
// título do gráfico
$grafico->title->Set("CENTRAL DE MONTAGEM  -  Emissão($data) \n\nO montador $nomeMontador possui $rows fichas \ncom data de recebimento entre os dias $data_inicio e $data_fim");
