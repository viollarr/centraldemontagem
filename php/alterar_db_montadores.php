...
if(strlen($_POST['atendimento'])>0){
	...
	for($i=0;$i<$rows;$i++){
		...
		$select_id_bairro = "SELECT id FROM RJ WHERE bairro = _UTF8 '".$atendimento2[$i]."' COLLATE utf8_unicode_ci  LIMIT 1";
		//echo $select_id_bairro;
		$query_id_bairro = mysql_query($select_id_bairro);
		$res_id_bairro = mysql_fetch_array($query_id_bairro);
		$atendimento .= $res_id_bairro['id'].';';
