...
$select = "
	...
	INNER JOIN 
		ordem_montagem o 
	ON 
		(c.n_montagem = o.n_montagem) 
	INNER JOIN 
		montadores m 
	ON 
		(o.id_montador = m.id_montadores) 
	INNER JOIN 
		estados es 
	ON 
		(c.estado_cliente = es.cod_estados) 
	INNER JOIN 
		cidades cd 
	ON 
		(c.cidade_cliente = cd.cod_cidades) 
	WHERE 
		c.n_montagem = '".$v_montagem."'
";
...
<title>CENTRAL DE MONTAGEM - ASSIST&Ecirc;NCIA T&eacute;cnica</title>
...
<table width="60%" align="center" cellpadding="1" cellspacing="1" border="1">
	...
	<tr>
        ...
        <td width="39%" align="left">CENTRAL DE MONTAGEM</td>
	...
	<tr>
        ...
        <td align="left"><?=htmlentities($x['nome'])?></td>
	...
	<tr>
    	...
    	<td align="left" bgcolor="#CAC7C6"><strong>LOJA:</strong></td>

        <td align="left" colspan="3"><?=$x['cod_loja']?></td>
	...
	<tr>
        ...
        <td align="left"><?=htmlentities($x['endereco_cliente'])?></td>
	...
	<tr>
        ...
        <td align="left"><?=htmlentities($x['bairro_cliente'])?></td>

        <td align="left" bgcolor="#CAC7C6"><strong>CIDADE:</strong></td>

        <td align="left"><?=strtoupper(strtr(strtoupper(utf8_encode(strtoupper(trim($x['cidade_cliente'])))) ,"áéíóúâêôãõàèìòùç","ÁÉÍÓÚÂÊÔÃÕÀÈÌÒÙÇ"))?></td>
	...
	<tr>
        ...
        <td align="left" colspan="3"><?=htmlentities($x['referencia_cliente'])?></td>
